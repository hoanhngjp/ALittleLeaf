...
- [X] Phase 14 — Cutover & Legacy Cleanup (14.2 ✓ webapp removed from docker-compose; 14.3 ✓ legacy ALittleLeaf/ MVC removed from sln; 14.4 ✓ Dockerfile.test removed; 14.5 ✓ CI finalized; 14.6 ✓ README updated)
- [X] Phase 15 — Google SSO (AuthProvider/GoogleId on User; UserPassword nullable; Google.Apis.Auth; GoogleLoginAsync with EmailVerified gate + 3-case merge; POST /api/auth/google-login; @react-oauth/google on Login+Register pages)
- [X] Phase 16 — Inventory Reservation & Auto-Cancel (RowVersion on Product; CreatedAtTime on Bill; stock deducted at CreateOrderAsync with DbUpdateConcurrencyException; ExpiredOrderCancellationService BackgroundService every 5 min restores stock + cancels stale VNPay orders)
- [X] Phase 17 — Test Suite (IGoogleTokenValidator interface extracted; AuthServiceTests 3 SSO scenarios; OrderServiceTests +2 concurrency scenarios; ExpiredOrderCancellationServiceTests 2 background job scenarios; 20/20 passing)
- [X] Phase 18 — Dynamic Banner Management (Banner entity + EF migration + HasData seed; IBannerRepository/BannerRepository with Take(5) public cap; IBannerService/BannerService with R1 file validation, R2 f_auto,q_auto URL rewrite, Cloudinary delete-on-remove; UploadImageWithPublicIdAsync added to ICloudinaryService; GET /api/banners public + full admin CRUD at /api/admin/banners; useBanners.js React Query hooks; HeroSlider dynamic with R4 static fallback; AdminBannersPage with upload form + inline-edit table + active toggle)
